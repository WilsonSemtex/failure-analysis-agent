...
def _load_binary_office_text(file_path: str) -> list:
    """旧版二进制 Word/PPT（.doc/.ppt）：尽力提取 UTF-16LE 编码文本。

    .doc/.ppt 正文通常以 UTF-16LE 存储于二进制流中，直接解码后清理控制字符。
    属于尽力而为的提取（无外部依赖），内容主体可用，可能夹杂少量乱码。
    """
    from langchain_core.documents import Document
    import re
    ...
    text = re.sub(r"\n{3,}", "\n\n", text).strip()
    if not text:
        text = f"[未能从 {os.path.basename(file_path)} 中提取文本内容，该文件可能是加密或非文本格式。]"
...
def load_document(file_path: str) -> list:
    """
    根据文件类型加载文档
    支持：PDF、TXT、MD、DOCX、DOC、PPTX、PPT、XLSX、XLS、图片(PNG/JPG/JPEG/GIF/BMP/WebP)
    ...
    DOC/PPT 为旧版二进制格式，使用 UTF-16LE 尽力提取文本。
    """
    ext = os.path.splitext(file_path)[1].lower()
    ...
    if ext == ".pdf":
        ...
        return _load_binary_office_text(file_path)
    elif ext in (".png", ".jpg", ".jpeg", ".gif", ".bmp", ".webp"):
        return _load_image_as_document(file_path)
    else:
        raise ValueError(f"不支持的文件格式: {ext}，仅支持 PDF/TXT/MD/DOCX/DOC/PPTX/PPT/XLSX/XLS/图片")
